...
async function server (dir, options) {
  ...
  app.use(staticHandler)
      ...
      .use(serveIndex(root, { icons: true , view: 'details', template: path.join(__dirname, 'index.html')}));
  ...
  io.on('connection', (socket) => {
   console.log(chalk.green('ws 连接成功！'));
    sockets = socket;
    // 断开连接
    socket.on('disconnect', () => {
        console.log(chalk.green('ws 连接断开！'));
    })
  ...
  watcher
    .on("change", path => {
      console.log(`Directory ${path} has been changeed`);
      sockets.emit('reload');
    })
    .on("add", path => {
